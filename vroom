...
class Person {
  ...
  void buyCar(Car car) {
    if (moneyLeft >= car.price) {
      ...
      print('Oops! $name, you don’t have enough money to buy a ${car.name}.Better luck next time.');
    }
  }
  ...
  while (true) {
    ...
    if (choice == 1) {
      ...
      if (carChoice == 1) {
        ...
        print('SOORY! That’s a wrong choice. Try again.');
      }
    } 
else if (choice == 3) {
      if (user.ownedCars.isNotEmpty) {
        ...
        if (sellChoice >= 1 && sellChoice <= user.ownedCars.length) {
          ...
          print('Hmm… That’s  a wrong choice. Try again.');
        }
      } 
else {
      ...
      if (user.ownedCars.isNotEmpty) {
        print('\nYour owned cars, $userName:');
        for (int i = 0; i < user.ownedCars.length; i++) {
          print('${i + 1}. ${user.ownedCars[i].name}');
        }
      } 
else {
        print('$userName, you don’t own any cars yet!');
      }
    } 
else if (choice == 5) {
      print('Thanks for visiting the Car Buying and Selling Platform, $userName! Drive safe!.Have great journey!');
      break;
    } 
else {
